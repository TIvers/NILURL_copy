...
$code = $request_uri;
$allowed_codes = ['main', 'registration', 'login', 'price', 'setting', 'links', 'graph','FAQ', 'favicon.ico', 'index.html', 'static/css/main.185480fe.css', 'static/js/main.40556f1c.js', 'test.svg', "/favicon.ico HTTP/1.1"];
...
if ($code_next) {
    if ($row_date && strtotime($date_time) <= strtotime(date('Y-m-d'))) {
        header("Location: https://nilurl.ru/error");
        exit();
    } else {
        ...
        if ($row) {
            new_redirect($pdo, $date, $browser, $OS, $city, $country, $country_code, $device, $redirect, $code);
            ...
            if ($OS === 'Android' && !empty($row['android']) && $row['android'] !== "false") {
                if ($utm_android) {
            ...
            elseif ($OS === 'iPhone' && !empty($row['ios']) && $row['android'] !== "false") {
                if ($utm_ioc) {
            ...
            header("Location: https://nilurl.ru/main");
            exit();
        }
    }
} else {
