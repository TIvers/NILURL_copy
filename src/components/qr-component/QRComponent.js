...
import { QRImage, Toggle, ColorPickerGfg, UpgradeToProPopupOverlay } from "../../components";
import "./qrComponent.css";

function QRComponent({ pathS, qr_open }) {
  const [showLogo, setShowLogo] = useState(true);
  const [isEditing, setIsEditing] = useState(false);
  const [borderColor, setBorderColor] = useState("#000000");
  const [setShowPopup] = useState(false);
  const [showQRCode, setShowQRCode] = useState(false);
  const [availableQR, setAvailableQR] = useState(null);

  const qrRef = useRef(null);
  const colorPickerRef = useRef(null); 

  useEffect(() => {
    const fetchAvailableQR = async () => {
      try {
        const response = await fetch('https://nilurl.ru:8000/get_available_qr.php', {
          method: 'POST',
          credentials: 'include' 
        });
        ...
        if (data.status === 'success') {
          setAvailableQR(data.available_qr);
        } else {
  ...
  const handleGenerateQRCode = async () => {
    if (availableQR > 0) {
      try {
        const response = await fetch('https://nilurl.ru:8000/reduce_available_qr.php', {
          method: 'POST',
          credentials: 'include',
          headers: {
        ...
        console.error('Error reducing available QR codes:', error);
      }
    } else {
      console.error('No QR codes available.');
    }
  };
...
{!showQRCode && availableQR !== null && (
        <UpgradeToProPopupOverlay onClick={showQRCodeHandler} onClose={handlePopupClose} style={{ width: "250px" }}>
          <p className="popup-message">
            Создать QR код для данной ссылки? Количество созданных QR кодов ограничено. Доступно: {availableQR}.
          </p>
          <button
            className="popup-button"
            onClick={handleGenerateQRCode} 
            disabled={availableQR <= 0} 
          >
            Создать QR код
          </button>
        </UpgradeToProPopupOverlay>
      )}
    </>
  );
}
