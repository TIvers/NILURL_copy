...
const CreatingLink = () => {
  ...
  const [isPro] = useState(isPremium !== 'free' && isPremium !== 'base');
  ...
  useEffect(() => {
    ...
    const anyToggleChecked = toggles.some((toggle) => toggle.checked);
  

    setIsExpanded(anyToggleChecked);
  }, [toggles]); 
  ...
  const generateShortUrl = (input) => {
    const uniqueInput = `${input}-${Date.now()}-${uuidv4()}`;
    const hash = CryptoJS.SHA256(uniqueInput).toString();
  ...
  return (
    <div className="creating__link">
      ...
      {isLinkPopupActive && (
        <UpgradeToProPopup onClose={closeShortLinkPopup}>
          ...
          {toggles.map((toggle, index) => (
            <div className="link__functional-item" key={toggle.id}>
              ...
              {toggle.checked !== undefined && toggle.checked && (
                <div className={`functional__item-info ${isExpanded ? 'expand' : ''}`}
                ref={contentRef}>{toggle.info}</div>
              )}
...
const CommentComponent = ({ commentError }) => {
  ...
  return (
    <div className="">
      <div
        className="w-full rounded flex flex-col space-y-2"
        style={{ marginBottom: "10px" }}
      >
        <textarea
          className={
            commentError
              ? "p-2 outline-none rounded border border-gray-300 custom-textarea input-error"
              : "p-2 outline-none rounded border border-gray-300 custom-textarea"
          }
  ...
  return (
    <div>
      <input
        ...
        type="text"
        placeholder="https://apps.apple.com/app/1833162974"
  ...
  return (
    <div>
      <input
        ...
        type="text"
        placeholder="https://play.google.com/store/apps/details?id=148362974"
