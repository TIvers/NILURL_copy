...
try {
    ...
    if ($result && pg_num_rows($result) > 0) {
        while ($row = pg_fetch_assoc($result)) {
            ...
            $update_query = "UPDATE users SET available_links = $1, available_qr = $2 WHERE user_id = $3";
            $update_result = pg_query_params($conn, $update_query, array($available_links, $available_qr, $user_id));
